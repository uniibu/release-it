...
const runTasks = async (opts, injected = {}) => {
  ...
  try {
    ...
    if (isInteractive && !v.version) {
      await prompt.show({
        ...
        task: async increment =>
          increment ?
          await v.bump({ increment }) : await prompt.show({ prompt: 'version', task: async version => (v.version = version) })
      });
    }
    ...
    const step = options => ((isInteractive || options.prompt === 'publish') ? prompt.show(options) : spinner.show(options));
    ...
    const release = async ({ gitClient, ghClient, glClient, npmClient, scripts }) => {
      ...
      const otpCallback = task => prompt.show({ prompt: 'otp', task });
